...
/// injection surface. The two jsonb keys read (`cost`, `is_directed`) are fixed literals.
/// Checked in both `derived` (topic/category `consts`, derivers) and `osm` (`osm_fields`,
/// sanitizers) since a topic may define either field via either mechanism.
...
fn create_topic_edge_table_sql(table: &str, mode: TopicEdgeMode) -> String {
    let extra_columns = match mode {
        ...
        TopicEdgeMode::All => ", t.osm_type, t.id, t.osm, t.derived, t.private, t.meta",
    };
    format!(
        r#"
...
SELECT e.osm_id, e.seg_idx, e.start_id, e.end_id, e.geom, e.length_m, e.total_length_m,
       COALESCE(t.derived->>'cost', t.osm->>'cost')::double precision
         * (e.length_m / NULLIF(e.total_length_m, 0)) AS cost,
       CASE WHEN COALESCE((t.derived->>'is_directed')::boolean, (t.osm->>'is_directed')::boolean, false) THEN -1
            ELSE COALESCE(t.derived->>'cost', t.osm->>'cost')::double precision
                 * (e.length_m / NULLIF(e.total_length_m, 0))
       END AS reverse_cost{extra_columns}
FROM {EDGE_TABLE} e
JOIN {table} t ON t.osm_id = e.osm_id
WHERE COALESCE(t.derived ? 'cost', false) OR COALESCE(t.osm ? 'cost', false)
"#
    )
}
