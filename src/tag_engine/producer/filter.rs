...
//! `producer/mod.rs`) with `Producer::eval` — a predicate is just another "object state → output"
...
//! load time (`expand`, in `tag_engine::loader::filter`) before `eval` ever runs. The category
...
use crate::tag_engine::producer::keys::first_present;
use crate::tag_engine::producer::{resolve_sanitize, ExtractCtx, SanitizeRef};
