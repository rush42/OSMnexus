...
//! same way with input size (kept relations × members), so it gets the same `Memory`/`Disk` split
//! and the same `--use-disk-store` opt-in, built on the same `ArenaStore`/`ArenaBuilder` backend
//! `way_refs`' `Disk` variant uses (see `store`'s own doc for the shared mechanics).
//!
...
use super::store::ArenaStore;
use super::way_refs::{read_varint, write_varint, zigzag_decode, zigzag_encode};

/// Delta+zigzag+varint encoded `(way_id, role)` list. `encode`/`decode` are the only way in/out —
...
/// `SelectionContext::rel_members`'s storage: `Memory` (default, a resident `FxHashMap`) or `Disk`
/// (the `--use-disk-store` opt-in). Exposes the two access patterns `geom::materialize` actually
/// needs (every member way id across all relations, and the full per-relation request list) instead
/// of a generic iterator — same reasoning as `WayRefsStore`'s own purpose-built methods.
pub enum RelMembers {
    Memory(FxHashMap<i64, (Vec<(i64, MemberRole)>, u32)>),
    Disk(ArenaStore),
}

impl RelMembers {
    pub fn build(map: FxHashMap<i64, (Vec<(i64, MemberRole)>, u32)>, disk: bool) -> anyhow::Result<Self> {
        if disk {
            let records: Vec<(i64, Box<[u8]>, u32)> = map
                .into_iter()
                .map(|(id, (members, mask))| (id, EncodedMembers::encode(&members).as_bytes().into(), mask))
                .collect();
            Ok(RelMembers::Disk(ArenaStore::build(records)?))
        } else {
            Ok(RelMembers::Memory(map))
        }
    }

    pub fn is_empty(&self) -> bool {
        match self {
            RelMembers::Memory(m) => m.is_empty(),
            RelMembers::Disk(d) => d.len() == 0,
        }
    }

    pub fn len(&self) -> usize {
        match self {
            RelMembers::Memory(m) => m.len(),
            RelMembers::Disk(d) => d.len(),
        }
    }

    /// Every member way id across every relation, deduplicated.
    pub fn member_way_ids(&self) -> rustc_hash::FxHashSet<i64> {
        match self {
            RelMembers::Memory(m) => {
                m.values().flat_map(|(members, _)| members.iter().map(|&(w, _)| w)).collect()
            }
            RelMembers::Disk(d) => {
                d.iter().flat_map(|(_, bytes, _)| decode_members(bytes).into_iter().map(|(w, _)| w)).collect()
            }
        }
    }
    ...
    pub fn requests(&self) -> Vec<(i64, Vec<(i64, MemberRole)>, u32)> {
        match self {
            RelMembers::Memory(m) => m.iter().map(|(&id, (members, mask))| (id, members.clone(), *mask)).collect(),
            RelMembers::Disk(d) => d.iter().map(|(id, bytes, mask)| (id, decode_members(bytes), mask)).collect(),
        }
    }
}
...
mod tests {
    ...
    fn encoded_members_round_trips() {
        ...
        assert_eq!(decode_members(encoded.as_bytes()), members);
    }

    #[test]
    fn disk_backed_rel_members_round_trips_present_ids_and_rejects_absent_ones() {
        ...
        let store = RelMembers::build(map, true).unwrap();
