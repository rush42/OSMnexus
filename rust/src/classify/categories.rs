...
pub struct CategoriesFile {
    ...
    /// Discrimination net over `order` (see `decision_tree`): prunes the first-match walk to a small
    /// per-object candidate set. Built by `build_order` after `order`. Not part of the JSON.
    #[serde(skip)]
    pub tree: crate::classify::decision_tree::DecisionTree,
}
...
impl CategoriesFile {
    ...
    pub fn build_order(&mut self) -> anyhow::Result<()> {
        ...
        self.order = order;
        // Compile the discrimination net over the finished order (needs `order` + merged macros).
        self.tree = crate::classify::decision_tree::build(self);
        Ok(())
    }
}
